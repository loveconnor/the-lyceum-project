...
exports.filterNonLearningContent = filterNonLearningContent;
exports.selectNodesForModule = selectNodesForModule;
exports.resolveNodesForModule = resolveNodesForModule;
 ...
 */
function filterNonLearningContent(summaries) {
    ...
    return summaries.filter(summary => {
        ...
        if (shouldExclude) {
            logger_1.logger.info('node-resolver', `Filtering out non-learning content: "${summary.title}"`);
        }
 ...
 */
async function selectNodesForModule(moduleTitle, moduleDescription, tocNodes, pathContext) {
    const startTime = Date.now();
    logger_1.logger.info('node-resolver', `Selecting nodes for module: "${moduleTitle}"`, {
    ...
    if (filteredSummaries.length === 0) {
        logger_1.logger.warn('node-resolver', 'No learning content nodes available after filtering');
        return {
            selected_node_ids: [],
            reasoning: 'All available nodes were setup/administrative content',
        };
    }
    logger_1.logger.info('node-resolver', `Filtered ${summaries.length - filteredSummaries.length} non-learning nodes`);
...
Select the most appropriate sections for this module.`;
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.error('node-resolver', 'Failed to parse AI response', {
        ...
        if (validatedIds.length !== result.selected_node_ids?.length) {
            logger_1.logger.warn('node-resolver', 'Some selected node IDs were invalid or filtered out', {
                details: {
        ...
        logger_1.logger.info('node-resolver', `Node selection complete for "${moduleTitle}"`, {
            duration,
            details: {
        ...
        logger_1.logger.error('node-resolver', `AI node selection failed: ${error.message}`, {
            details: { moduleTitle },
        });
        throw error;
    }
 ...
 */
async function resolveNodesForModule(request, tocNodes) {
    const startTime = Date.now();
    logger_1.logger.info('node-resolver', `Resolving nodes for module: "${request.module_title}"`, {
        details: {
    ...
    if (!tocNodes || tocNodes.length === 0) {
        logger_1.logger.warn('node-resolver', 'No TOC nodes available for asset', {
    ...
    try {
        const selectionResult = await selectNodesForModule(request.module_title, request.module_description, tocNodes, request.path_context);
        const contentUnavailable = selectionResult.selected_node_ids.length === 0;
        const duration = Date.now() - startTime;
        logger_1.logger.info('node-resolver', `Node resolution complete for "${request.module_title}"`, {
            duration,
            details: {
        ...
        logger_1.logger.error('node-resolver', `Node resolution failed: ${error.message}`, {
        ...
        return {
            ...
            source_node_ids: [],
            content_unavailable: true,
            reasoning: `Node resolution failed: ${error.message}`,
            resolved_at: new Date().toISOString(),
        };
    }
}
