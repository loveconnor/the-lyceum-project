...
exports.KNOWN_DOC_SOURCES = [
    // Programming Languages
    {
        ...
        description: 'Official Oracle Java Tutorials - comprehensive Java learning trail',
    ...
    },
    {
        ...
        type: 'generic',
        keywords: ['java', 'javase', 'jdk', 'api', 'reference'],
        description: 'Official Java SE API documentation and reference',
        license: 'Oracle Technology Network License',
        licenseUrl: 'https://www.oracle.com/legal/terms.html',
    },
    {
        ...
        type: 'generic',
        keywords: ['java', 'programming', 'beginner', 'tutorial'],
...
class WebDocsSearcher {
     ...
     */
    searchKnownDocs(query) {
        const queryTerms = this.tokenize(query);
        const results = [];
        for (const source of exports.KNOWN_DOC_SOURCES) {
        ...
        logger_1.logger.info('web-docs', `Found ${results.length} matching docs for "${query}"`, {
    ...
    async searchWeb(query) {
        ...
        logger_1.logger.info('web-docs', `Searching web for: "${query}"`);
        console.log(`[WebDocs] Searching DuckDuckGo for: "${searchQuery}"`);
        try {
            const result = await fetcher_1.fetcher.fetch(searchUrl, { timeout: 10000 });
            if (!result.ok || !result.html) {
                logger_1.logger.warn('web-docs', 'Web search failed', { details: { error: result.error } });
                return [];
            }
            ...
            logger_1.logger.info('web-docs', `Web search returned ${results.length} results`);
            return results;
        }
        catch (error) {
            logger_1.logger.error('web-docs', `Web search error: ${error.message}`);
            return [];
        }
    }
    /**
     ...
     */
    async getOrCreateDocAsset(docSource) {
        ...
        logger_1.logger.info('web-docs', `Created asset for ${docSource.name}`, { details: { assetId: created.id } });
        return created;
    }
    /**
     * Get or create source for a documentation site
     */
    async getOrCreateDocSource(docSource) {
     ...
     */
    async getTocForDocAsset(asset, docSource) {
        ...
        logger_1.logger.info('web-docs', `Fetching TOC for ${asset.title}`);
        const candidate = {
            slug: asset.slug,
            title: asset.title,
            url: docSource.docUrl,
        ...
        if (docSource.type === 'sphinx') {
            tocNodes = await adapters_1.sphinxDocsAdapter.mapToc(candidate, docSource.baseUrl);
        }
        else {
            tocNodes = await adapters_1.genericHtmlAdapter.mapToc(candidate, docSource.baseUrl);
        }
     ...
     */
    async extractBasicStructure(docSource) {
        console.log(`[WebDocs] Attempting basic structure extraction`);
        const result = await fetcher_1.fetcher.fetch(docSource.docUrl);
        if (!result.ok || !result.html)
            return [];
        const cheerio = await Promise.resolve().then(() => __importStar(require('cheerio')));
        const $ = cheerio.load(result.html);
        ...
        const addNode = (href, title, depth = 1) => {
            if (!href || !title || title.length > 100 || title.length < 3)
                return false;
            let fullUrl;
            try {
        ...
        for (const selector of mainContentSelectors) {
            $(selector).each((_, el) => {
                ...
                if (title.length < 5)
                    return;
                if (['home', 'back', 'next', 'previous', 'skip'].some(w => title.toLowerCase() === w))
                    return;
                addNode(href, title);
            });
            if (nodes.length > 10)
        ...
        if (nodes.length < 5) {
            $('.mw-parser-output > ul a, .mw-parser-output > ol a').each((_, el) => {
                ...
                if (href.includes('action=edit') || href.startsWith('http') && !href.includes('wikibooks'))
                    return;
    ...
    async saveTocToRegistry(assetId, nodes) {
        ...
        for (let i = 0; i < nodesToInsert.length; i += batchSize) {
            ...
            const { error } = await this.supabase
                ...
                .insert(batch);
            if (error) {
                logger_1.logger.error('web-docs', `Failed to save TOC: ${error.message}`);
                throw error;
            }
        }
        console.log(`[WebDocs] ✓ Saved ${nodes.length} TOC nodes to database`);
        logger_1.logger.info('web-docs', `Saved ${nodes.length} TOC nodes`);
    }
    /**
     ...
     */
    async discoverDocsForTopic(topic) {
        logger_1.logger.info('web-docs', `Discovering docs for: "${topic}"`);
        ...
        for (const match of viableDocs) {
            ...
            try {
                ...
                console.log(`[WebDocs] ⚠️ Error with ${docSource.name}: ${error.message}`);
                logger_1.logger.error('web-docs', `Error fetching ${docSource.name}: ${error.message}`);
            }
        }
        ...
        const docResult = webResults.find(r => docDomains.some(d => r.domain.includes(d) || r.url.includes(d)));
        if (docResult) {
            console.log(`[WebDocs] Found web documentation: ${docResult.title}`);
            logger_1.logger.info('web-docs', `Found web doc: ${docResult.url}`);
        }
        return null;
    }
    /**
     ...
     */
    calculateDocMatch(source, queryTerms) {
        ...
        for (const term of queryTerms) {
            ...
            if (hasConflict) {
                ...
                logger_1.logger.debug('web-docs', `Skipping "${source.name}" - term "${term}" conflicts with keywords`);
...
exports.WebDocsSearcher = WebDocsSearcher;
