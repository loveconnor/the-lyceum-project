...
exports.synthesizeModuleContent = synthesizeModuleContent;
exports.renderModuleOnDemand = renderModuleOnDemand;
 ...
 */
function formatSourceContentForPrompt(contents) {
    const parts = [];
 ...
 */
async function synthesizeModuleContent(moduleTitle, moduleDescription, extractedContents, difficulty) {
    const startTime = Date.now();
    logger_1.logger.info('content-synthesizer', `Synthesizing content for module: "${moduleTitle}"`, {
        details: {
    ...
    if (extractedContents.length === 0) {
        logger_1.logger.warn('content-synthesizer', 'No extracted content provided for synthesis');
        return {
    ...
    // Build citations
    const citations = (0, module_content_retriever_1.buildCitations)(extractedContents);
...
Remember: ONLY use information present in the source material above. Do NOT add external knowledge.`;
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.error('content-synthesizer', 'Failed to parse AI response', {
        ...
        logger_1.logger.info('content-synthesizer', `Content synthesis complete for "${moduleTitle}"`, {
            duration,
            details: {
        ...
        logger_1.logger.error('content-synthesizer', `AI synthesis failed: ${error.message}`, {
            details: { moduleTitle },
        });
        return {
            ...
            rendered_at: new Date().toISOString(),
            content_unavailable: true,
            unavailable_reason: `Synthesis error: ${error.message}`,
        };
    }
}
/**
 ...
 */
async function renderModuleOnDemand(moduleTitle, moduleDescription, extractedContents, difficulty) {
    return synthesizeModuleContent(moduleTitle, moduleDescription, extractedContents, difficulty);
}
