...
exports.openStaxAdapter = exports.OpenStaxAdapter = void 0;
const base_adapter_1 = require("./base-adapter");
const fetcher_1 = require("../fetcher");
class OpenStaxAdapter extends base_adapter_1.BaseAdapter {
    constructor() {
        super(...arguments);
        this.sourceType = 'openstax';
        this.openstaxBaseUrl = 'https://openstax.org';
        this.cmsApiUrl = 'https://openstax.org/apps/cms/api';
        this.cnxBaseUrl = 'https://openstax.org/books';
    }
    /**
    ...
    async discoverAssets(seedUrl, config) {
        ...
        const result = await fetcher_1.fetcher.fetch(`${this.cmsApiUrl}/books/`);
        ...
        try {
            ...
            for (const book of books) {
                ...
                assets.push({
                    ...
                    metadata: {
                        ...
                        subjects: book.book_subjects?.map((s) => s.subject_name) || [],
                        categories: book.book_categories?.map((c) => c.subject_category) || [],
            ...
            throw new Error(`Failed to parse books API response: ${err.message}`);
        }
    ...
    async mapToc(asset, baseUrl) {
        ...
        try {
            ...
            this.log('error', 'map-toc', `Failed to parse __PRELOADED_STATE__: ${err.message}`);
            return [];
        }
        ...
        const parseTreeContents = (contents, depth) => {
            const levelNodes = [];
...
exports.OpenStaxAdapter = OpenStaxAdapter;
exports.openStaxAdapter = new OpenStaxAdapter();
