...
const sanitizeStringForDatabase = (input) => {
...
const sanitizeValueForDatabase = (value) => {
...
const sanitizePathItemsForInsert = (items) => {
    return items.map((item) => sanitizeValueForDatabase(item));
};
const clampInteger = (value, min, max) => {
    return Math.min(max, Math.max(min, Math.round(value)));
};
const sanitizeRequestedCount = (value, min, max) => {
...
const getRequestedLabInsertionIndices = (moduleCount, requestedLabCount) => {
...
const shouldInsertLabAfterModule = (modules, moduleIndex, includeLabs, requestedLabCount) => {
    if (!includeLabs)
        return false;
    const hasExplicitRequestedCount = Number.isInteger(requestedLabCount);
    if (!hasExplicitRequestedCount && moduleIndex >= modules.length - 1)
        return false;
    ...
    const hasExplicitLabPlacement = modules.some((entry) => typeof entry.include_lab_after === "boolean");
...
router.post("/generate", async (req, res) => {
    ...
    try {
        ...
        console.log(`[Generate] Structure controls: modules=${requestedModuleCount ?? 'auto'}, labs=${includeLabs ? (requestedLabCount ?? 'auto') : 'disabled'}`);
...
Remember: Output MUST be valid JSONL patches only. Start with {"op":"set","path":"/root","value":"..."}`;
            ...
            try {
                ...
                if (asset && tocSummaries.length > 0) {
                    ...
                    for (const moduleOutline of outline.modules) {
                        ...
                        if (shouldInsertLabAfterModule(outline.modules, moduleOutline.order_index, includeLabs, requestedLabCount) &&
        ...
        for (let i = 0; i < moduleItems.length; i++) {
            ...
            if (shouldInsertLabAfterModule(outline.modules, i, includeLabs, requestedLabCount) &&
                allPathItems[allPathItems.length - 1]?.item_type !== 'lab') {
                allPathItems.push({
                    path_id: newPath.id,
                    lab_id: null,
...
router.post("/generate-registry", async (req, res) => {
    ...
    try {
        ...
        console.log(`[Registry] Structure controls: modules=${requestedModuleCount ?? 'auto'}, labs=${includeLabs ? (requestedLabCount ?? 'auto') : 'disabled'}`);
