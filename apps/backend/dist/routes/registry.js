...
router.post('/scan', async (req, res) => {
    ...
    try {
        ...
        const service = new source_registry_1.RegistryService(supabase);
        ...
        if (source_id && typeof source_id === 'string') {
            ...
            source_registry_1.logger.info('api', `Scanning source by ID: ${source_id}${skipScanned ? ' (skip scanned)' : ''}`);
            ...
            const seed = (0, source_registry_1.getSeedByName)(seed_name);
            if (!seed) {
                return res.status(404).json({
                    error: `Seed not found: ${seed_name}`,
                    available_seeds: source_registry_1.SEED_SOURCES.map(s => s.name),
                });
            }
            source_registry_1.logger.info('api', `Scanning seed: ${seed_name}${skipScanned ? ' (skip scanned)' : ''}`);
            ...
            // Scan all seeds
            source_registry_1.logger.info('api', `Scanning all seeds${skipScanned ? ' (skip scanned)' : ''}`);
        ...
        source_registry_1.logger.error('api', `Scan failed: ${err.message}`);
...
router.get('/sources', async (req, res) => {
    try {
        ...
        return res.json({
            ...
            available_seeds: source_registry_1.SEED_SOURCES.map(s => ({
                name: s.name,
                type: s.type,
                base_url: s.baseUrl,
            })),
        ...
        source_registry_1.logger.error('api', `Failed to get sources: ${err.message}`);
        return res.status(500).json({
...
router.get('/sources/:id', async (req, res) => {
    try {
        ...
        source_registry_1.logger.error('api', `Failed to get source: ${err.message}`);
        return res.status(500).json({
...
router.get('/assets', async (req, res) => {
    try {
        ...
        let assets = await service.getAssets(typeof source_id === 'string' ? source_id : undefined);
        ...
        source_registry_1.logger.error('api', `Failed to get assets: ${err.message}`);
        return res.status(500).json({
...
router.get('/assets/:id', async (req, res) => {
    try {
        ...
        source_registry_1.logger.error('api', `Failed to get asset: ${err.message}`);
        return res.status(500).json({
 ...
 */
router.post('/activate-asset', async (req, res) => {
    try {
        const supabase = (0, supabaseAdmin_1.getSupabaseAdmin)();
        const service = new source_registry_1.RegistryService(supabase);
        ...
        source_registry_1.logger.info('api', `Activated asset: ${asset.title}`, { assetId: asset.id });
        return res.json({
            success: true,
            asset,
        });
        ...
        source_registry_1.logger.error('api', `Failed to activate asset: ${err.message}`);
 ...
 * Body: { asset_id: string }
 */
router.post('/deactivate-asset', async (req, res) => {
    try {
        const supabase = (0, supabaseAdmin_1.getSupabaseAdmin)();
        const service = new source_registry_1.RegistryService(supabase);
        const { asset_id } = req.body;
        ...
        source_registry_1.logger.info('api', `Deactivated asset: ${asset.title}`, { assetId: asset.id });
        return res.json({
            success: true,
            asset,
        });
        ...
        source_registry_1.logger.error('api', `Failed to deactivate asset: ${err.message}`);
        return res.status(400).json({
...
router.get('/logs', async (req, res) => {
    try {
        ...
        source_registry_1.logger.error('api', `Failed to get logs: ${err.message}`);
        return res.status(500).json({
...
router.get('/seeds', async (req, res) => {
    return res.json({
        seeds: source_registry_1.SEED_SOURCES.map(s => ({
            name: s.name,
            type: s.type,
            base_url: s.baseUrl,
            seed_url: s.seedUrl,
