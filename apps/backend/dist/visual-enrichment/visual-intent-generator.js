...
exports.generateVisualIntent = generateVisualIntent;
exports.generateVisualIntentFromQuestion = generateVisualIntentFromQuestion;
exports.shouldUseVisualAids = shouldUseVisualAids;
 ...
 */
async function generateVisualIntent(request) {
    const startTime = Date.now();
    logger_1.logger.info('visual-intent', `Generating visual intents for module: "${request.module_title}"`, {
        details: {
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.error('visual-intent', 'Failed to parse AI response', {
        ...
        logger_1.logger.info('visual-intent', `Generated ${intents.length} visual intents`, {
            duration,
            details: {
        ...
        const duration = Date.now() - startTime;
        logger_1.logger.error('visual-intent', `Failed to generate visual intents: ${error.message}`, {
            duration,
        });
        // Graceful degradation - return empty result
        return {
            intents: [],
            reasoning: `Error generating visual intents: ${error.message}`,
            visuals_recommended: false,
            generated_at: new Date().toISOString(),
        };
    }
 ...
 */
function validateIntents(rawIntents) {
    ...
    for (const intent of rawIntents) {
        ...
        const keyElements = Array.isArray(intent.key_elements)
            ? intent.key_elements.filter((e) => typeof e === 'string')
            : [];
        ...
        const userConstraints = Array.isArray(intent.constraints)
            ? intent.constraints.filter((c) => typeof c === 'string')
            : [];
...
function sanitizeSearchQuery(query) {
    ...
    const hasDigramTerm = diagramTerms.some(term => cleaned.toLowerCase().includes(term));
    if (!hasDigramTerm) {
 ...
 */
async function generateVisualIntentFromQuestion(question) {
    const startTime = Date.now();
    logger_1.logger.info('visual-intent', `Generating visual intents for question: "${question.substring(0, 100)}..."`);
    try {
        const client = ensureClient();
        const model = USE_OLLAMA ? OLLAMA_MODEL : OPENAI_MODEL;
...
- "How do I feel better?" → visuals_recommended: false (not educational/visual)`;
        ...
        try {
            ...
            logger_1.logger.error('visual-intent', 'Failed to parse question visual intent response', {
                details: { response: rawResponse.substring(0, 500) },
            });
            return {
                intents: [],
        ...
        if (!parsed.visuals_recommended) {
            const duration = Date.now() - startTime;
            logger_1.logger.debug('visual-intent', `No visuals recommended for question`, {
                duration,
        ...
        const duration = Date.now() - startTime;
        logger_1.logger.info('visual-intent', `Generated ${intents.length} visual intents for question`, {
            duration,
            details: {
        ...
        const duration = Date.now() - startTime;
        logger_1.logger.error('visual-intent', `Failed to generate visual intents for question: ${error.message}`, {
            duration,
        });
        return {
            intents: [],
 ...
 */
async function shouldUseVisualAids(moduleTitle, moduleDescription) {
    const startTime = Date.now();
    logger_1.logger.debug('visual-intent', `Checking if module would benefit from visuals: "${moduleTitle}"`);
    try {
        const client = ensureClient();
        const model = USE_OLLAMA ? OLLAMA_MODEL : OPENAI_MODEL;
...
- Pure definitions or abstract theory → likely NO`;
        ...
        const duration = Date.now() - startTime;
        logger_1.logger.debug('visual-intent', `Visual aids check completed`, {
            duration,
        ...
        const duration = Date.now() - startTime;
        logger_1.logger.warn('visual-intent', `Failed to check visual aids: ${error.message}`, {
            duration,
        });
