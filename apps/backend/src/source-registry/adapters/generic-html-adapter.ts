...
interface GenericHtmlConfig {
  // Selectors for finding TOC elements
  tocSelector?: string;
  tocItemSelector?: string;
  tocLinkSelector?: string;
  
  // Selectors for content extraction hints
  contentSelector?: string;
  titleSelector?: string;
  
  // Asset discovery
  assetLinkSelector?: string;
  assetTitleSelector?: string;
...
const DEFAULT_CONFIG: GenericHtmlConfig = {
...
export class GenericHtmlTocAdapter extends BaseAdapter {
  readonly sourceType: SourceType = 'generic_html';
  private config: GenericHtmlConfig;

  constructor(config?: GenericHtmlConfig) {
    super();
   ...
   */
  setConfig(config: Partial<GenericHtmlConfig>): void {
  ...
  async discoverAssets(seedUrl: string, config?: Record<string, unknown>): Promise<AssetCandidate[]> {
    ...
    $(linkSelector).each((_, el) => {
      ...
      if (mergedConfig.excludePatterns?.some(p => href.includes(p))) return;
   ...
   */
  private extractFromTocContainer(
    $: cheerio.CheerioAPI,
  ...
  private extractFromLinks(
    ...
    for (const selector of navSelectors) {
      $(selector).each((_, el) => {
        ...
        if (this.config.excludePatterns?.some(p => href.includes(p))) return;
   ...
   */
  private getNodeType(depth: number): NodeType {
    switch (depth) {
      case 0:
        return 'chapter';
      case 1:
   ...
   */
  getSelectorHints(): SelectorHints {
    return {
