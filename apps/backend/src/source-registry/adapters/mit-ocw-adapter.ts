...
export class MitOcwAdapter extends BaseAdapter {
  readonly sourceType: SourceType = 'mit_ocw';
  
  private readonly ocwBaseUrl = 'https://ocw.mit.edu';
  ...
  private readonly curatedCourses = [
    {
      courseNumber: '6.0001',
  ...
  async discoverAssets(seedUrl: string, config?: Record<string, unknown>): Promise<AssetCandidate[]> {
    ...
    const assets: AssetCandidate[] = this.curatedCourses.map(course => {
  ...
  async mapToc(asset: AssetCandidate, baseUrl: string): Promise<TocNode[]> {
    ...
    if ($nav && $nav.length > 0) {
      ...
      const parseNavItems = async ($items: cheerio.Cheerio<any>, depth: number): Promise<TocNode[]> => {
   ...
   */
  private shouldExpandNode(node: TocNode): boolean {
    const title = node.title.toLowerCase();
    return (
      title.includes('lecture') ||
      title.includes('assignment') ||
      title.includes('reading')
    ) && node.url.includes('/pages/');
  }

  /**
   * Expand an index page (like /pages/lecture-notes/) to individual lecture nodes
   */
  private async expandIndexPage(indexNode: TocNode, assetSlug: string): Promise<TocNode[]> {
    const result = await fetcher.fetch(indexNode.url);
    if (!result.ok || !result.html) {
...
export const mitOcwAdapter = new MitOcwAdapter();
