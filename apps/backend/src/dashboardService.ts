...
export async function updateDashboardActivity(
  ...
  // Update top topics
  const top_topics: DashboardTopic[] = Array.isArray(dashboardState.top_topics)
    ...
    for (const topic of activityData.topics) {
      ...
      if (existingIndex >= 0) {
        ...
        if (activityData.activityType.includes('completed')) {
          top_topics[existingIndex].progress = Math.min(
            100,
            (top_topics[existingIndex].progress || 0) + 10
          );
        }
      } else {
        top_topics.push({
 ...
 */
export async function getLabsStatistics(userId: string) {
  const supabase = getSupabaseAdmin();
 ...
 */
export async function getPathsStatistics(userId: string) {
  const supabase = getSupabaseAdmin();
