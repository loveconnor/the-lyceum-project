...
export function ReactFlowWidget({
  ...
  const [localViewedVisuals, setLocalViewedVisuals] = useState<Set<number>>(new Set());
  ...
  // Completion check
  useEffect(() => {
    if (viewedVisuals.size >= visuals.length && visuals.length > 0 && onViewComplete) {
      onViewComplete();
    }
  ...
  useEffect(() => {
    ...
    const newNodes: Node[] = layoutedNodesData.map((node) => ({
      ...
      style: node.style ? { backgroundColor: 'transparent', border: 'none', width: 'auto' } : { backgroundColor: 'transparent', border: 'none', width: 'auto' },
    }));
       ...
       <div className="flex items-start justify-between px-1">
          <div>
            <div className="flex items-center gap-2 mb-1">
               <h3 className="text-lg font-semibold tracking-tight">{currentVisual.title}</h3>
            </div>
  ...
  return (
    <div 
      className={cn(
        ...
        selected ? "border-primary ring-2 ring-primary/20 shadow-lg scale-105" : "border-border hover:border-primary/50 hover:shadow-md"
      )}
    >
      <Handle 
        type="target" 
      ...
      <div className="flex flex-col gap-2">
        <div className="text-center">
          <div className="text-sm font-semibold leading-tight text-foreground">
             <Markdown 
                className="prose-sm dark:prose-invert prose-p:m-0 prose-p:leading-tight [&_*]:text-inherit pointer-events-none"
             >
