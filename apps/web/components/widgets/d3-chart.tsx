...
export function D3Chart({ options, height }: D3ChartProps) {
  ...
  useEffect(() => {
    ...
    if (hasFunctionSeries) {
      ...
      const firstFunctionSeries = series.find((s: any) => s.type === 'function');
    ...
    const xKey = firstSeries.xKey || (firstSeries.type === 'function' ? 'x' : firstSeries.xKey);
    ...
    const allYValues: number[] = series.flatMap((s: any) => {
      if (s.type === 'function') {
    ...
    const yMin: number = (typeof yMinValue === 'number' && isFinite(yMinValue)) ? yMinValue : 0;
    const yMax: number = (typeof yMaxValue === 'number' && isFinite(yMaxValue)) ? yMaxValue : 10;
    ...
    series.forEach((s: any, index: number) => {
      ...
      if (type === 'bar') {
        ...
        const functionData = generateFunctionData(functionExpression, xMin, xMax, numPoints);
        ...
        const xValues: string[] = Array.from(new Set(data.map((d: any) => String(d[xKey]))));
        const yValues: string[] = Array.from(new Set(data.map((d: any) => String(d[yKey]))));
